<?php
add_action('wp_ajax_process_jawali_payment', 'process_jawali_payment');
add_action('wp_ajax_nopriv_process_jawali_payment', 'process_jawali_payment');

function process_jawali_payment() {
    ...
    if (curl_errno($curl)) {
        error_log('Jawali Payment Error: ' . curl_error($curl));
        ...
        if (isset($response_data['responseStatus']['systemStatus']) && $response_data['responseStatus']['systemStatus'] == "0") {
            ...
            error_log('Jawali Payment Error: ' . $error_message);
